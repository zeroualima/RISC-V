...
/*  Fonction non feuille : Besoin de sauvegarder ra et s (2 mots)
    => PILE + 24
    s : registres a0 et a1 et pile aux adresses sp et sp+8, paramètre de type
        (struct structure_t) dont le champ entier de type (int64_t) est dans a0
        et en pile à l'adresse sp et le champ ptr de type (char *) est dans a1
        et en pile à l'adresse sp+8.

DEBUT DU CONTEXTE
  Fonction :
    affichage : non feuille
  Contexte :
    s.entier  : registre a0 ; pile *(sp+0)  # champ de type (int64_t)
    s.ptr     : registre a1 ; pile *(sp+8)  # champ de type (char *)
    ra        : pile *(sp+16)
FIN DU CONTEXTE */
affichage:
    addi sp, sp, -24
...
/*
  ...
  Contexte :
    e   : registre a0  # argument de type (int64_t)
